...
namespace TravailFinSession.Pages.Employe
{
    ...
    /// </summary>
    public sealed partial class ModifieEmployePage : Page
    {
        private Classes.Employe employeAModifier;
        public ModifieEmployePage()
        {
            InitializeComponent();
        }
        ...
        protected override void OnNavigatedTo(NavigationEventArgs e)
        {
            ...
            if (e.Parameter is Classes.Employe)
            {
                employeAModifier = e.Parameter as Classes.Employe;

                TbxMatricule.Text = employeAModifier.Matricule;
                TbxNom.Text = employeAModifier.Nom;
                TbxPrenom.Text = employeAModifier.Prenom;
                dpDateNaissance.Date = employeAModifier.DateNaissance;
                TbxEmail.Text = employeAModifier.Email;
                TbxAdresse.Text = employeAModifier.Adresse;
                dpDateEmbauche.Date = employeAModifier.DateEmbauche;
                TbxTauxHoraire.Text = employeAModifier.TauxHoraire.ToString();
                TbxUrl.Text = employeAModifier.Photo;

                if (employeAModifier.StatutEmploye == StatutEmploye.Journalier)
                {
                    cmbxStatut.SelectedIndex = 0;
                }
                else
                {
                    cmbxStatut.SelectedIndex = 0;
        ...
        private async void Modifier_Employe(object sender, RoutedEventArgs e)
        {
            bool valide = true;
            ...
            if (valide)
            {
                ...
                string resultat = SingletonEmploye.getInstance().modifierEmploye(employeAModifier.Matricule, nom, prenom, email, adresse, tauxHoraire, photo, statut);
        ...
        {
            Frame.Navigate(typeof(ListeEmployePage));
        }
    }
}
